...
const ConfirmationPage = () => {
  ...
  // FLAG pour empêcher la double finalisation
  ...
  useEffect(() => {
    ...
    const finalize = async () => {
      if (hasFinalized.current) return; // Empêche la double exécution
      ...
      try {
        ...
        console.log("Données sauvegardées récupérées:", savedDetails);
        ...
        const response = await axios.post(`${GATEWAY_URL}/reservations/finalize`, {
          ...
          reservationDetails: savedDetails, // On envoie toutes les données sauvegardées
        });
  ...
  return (
    <div className="text-center p-8">
      <h1 className="text-3xl font-bold text-green-500 dark:text-green-400">Réservation Confirmée !</h1>
      <p className="mt-2 text-slate-800 dark:text-slate-200">Merci. Un e-mail de confirmation vous a été envoyé.</p>
