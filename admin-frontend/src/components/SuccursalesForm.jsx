// src/components/TaxesForm.jsx (Version Finale Corrigée)

import { useForm } from 'react-hook-form';
import { useEffect } from 'react';

const TaxesForm = ({ initialData, onSubmit, onCancel }) => {
  // On extrait les localités des données initiales pour remplir les champs séparément
  const defaultFormValues = initialData 
    ? { ...initialData, pays: initialData.localites?.[0]?.pays || '', province: initialData.localites?.[0]?.province || '' } 
    : {};
  ...
  // Si les données initiales changent (par exemple, en cliquant sur un autre bouton "modifier"), on met à jour le formulaire.
  useEffect(() => {
  ...
  const handleLocalSubmit = async (data) => {
    // 1. On sépare les données de la taxe et celles de la localité
    const { pays, province, ...taxeData } = data;

    // 2. On construit l'objet final avec la structure attendue par le backend
    const finalData = {
      ...taxeData,
      // On crée le tableau `localites`
      localites: [
        { pays, province }
      ]
    };
    
    // 3. On envoie cet objet bien formaté
    try {
