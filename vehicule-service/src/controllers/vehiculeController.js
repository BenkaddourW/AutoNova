...
exports.getVehicules = asyncHandler(async (req, res) => {
  ...
  if (succursaleId && succursaleId !== "any") {
    where.succursaleidsuccursale = succursaleId;
  }
...
exports.getPublicFilterOptions = asyncHandler(async (req, res) => {
  const [
    ...
    Vehicule.findAll({
      attributes: [
        [Sequelize.fn("DISTINCT", Sequelize.col("sieges")), "sieges"],
      ],
      raw: true,
    ...
    Vehicule.findAll({
      attributes: [
        [
          Sequelize.fn("DISTINCT", Sequelize.col("typeentrainement")),
          "typeentrainement",
        ],
      ],
      raw: true,
...
exports.searchAvailableVehicles = asyncHandler(async (req, res) => {
  ...
  if (idsuccursale) {
    ...
    try {
      const response = await axios.get(`${GATEWAY_URL}/succursales/find-ids`, {
        params: { pays, province, ville: cityToSearch },
      });
      ...
      return res
        .status(502)
        .json({ message: "Le service de succursales est indisponible." });
    }
  }
  ...
  // Filtres principaux
  if (marque && marque !== "any")
    vehicleWhereClause.marque = { [Op.iLike]: marque };
  if (succursaleIdsToFilter.length > 0) {
    vehicleWhereClause.succursaleidsuccursale = {
      [Op.in]: succursaleIdsToFilter,
    };
  } else if (pays || province || cityToSearch) {
    return res.json({ vehicles: [], total: 0 });
  }
  ...
  if (datedebut && datefin) {
    try {
      ...
      const response = await axios.post(
        `${GATEWAY_URL}/reservations/disponibilites`,
        {
      ...
      availableVehicles = potentialVehicles.filter((v) =>
        availableVehicleIds.has(v.idvehicule)
      );
    } catch (error) {
      console.error(
        "Erreur d'appel au service de réservations:",
        error.message
      );
      return res
        .status(502)
        .json({ message: "Le service de réservations est indisponible." });
    }
  }
  ...
  if (availableVehicles.length > 0) {
    try {
      const allSuccursaleIds = [
        ...new Set(availableVehicles.map((v) => v.succursaleidsuccursale)),
      ];
      const succursalesResponse = await axios.get(
        `${GATEWAY_URL}/succursales`,
        {
          params: { ids: allSuccursaleIds.join(",") },
        }
      );
      const succursaleMap = new Map(
      ...
      enrichedVehicles = availableVehicles.map((vehicle) => {
        ...
        vehicleJson.Succursale =
          succursaleMap.get(vehicle.succursaleidsuccursale) || null;
        return vehicleJson;
      });
    } catch (error) {
  ...
  const paginatedVehicles = enrichedVehicles.slice(
    Number(offset),
...
exports.getFeaturedVehicles = asyncHandler(async (req, res) => {
  try {
    const topIdsResponse = await axios.get(
      `${GATEWAY_URL}/reservations/stats/top-vehicles?limit=3`
    );
    ...
    const allSuccursaleIds = [
      ...new Set(featuredVehicles.map((v) => v.succursaleidsuccursale)),
    ];
    ...
    const sortedEnrichedVehicles = topVehicleIds
      .map((id) => enrichedVehicles.find((v) => v.idvehicule === id))
      .filter(Boolean);

    res.json(sortedEnrichedVehicles);
  } catch (error) {
    console.error(
      "Erreur lors de la récupération des véhicules en vedette:",
      error.message
    );
    res
      .status(500)
      .json({ message: "Impossible de récupérer les véhicules en vedette." });
  }
});
    ...
    const vehiculesDisponibles = vehiculesPotentiels
      ...
      .map((v) => {
        ...
        return {
          ...vehiculeJson,
          photoPrincipale:
            vehiculeJson.VehiculeImages &&
            vehiculeJson.VehiculeImages.length > 0
...
/**
 * Récupère les véhicules par leurs IDs.
 * 
 * @route GET /vehicules/ids
 * @query {string} ids - Liste d'IDs de véhicules, séparés par des virgules
 * @returns {Array} Liste des véhicules correspondants, chacun avec un tableau d'URLs d'images

 */
exports.getVehiclesByIds = asyncHandler(async (req, res) => {
  const ids = req.query.ids ? req.query.ids.split(",").map(Number) : [];
  if (!ids.length) return res.status(400).json({ message: "Aucun ID fourni." });

  // On inclut les images associées à chaque véhicule
  const vehicules = await Vehicule.findAll({
    where: { idvehicule: ids },
    include: [
      {
        model: VehiculeImage,
        as: "VehiculeImages",
  ...
  // On formate la réponse pour inclure un tableau d'URLs d'images pour chaque véhicule
  const result = vehicules.map((v) => {
    const vehiculeJson = v.toJSON();
    return {
      ...vehiculeJson,
      images: vehiculeJson.VehiculeImages
        ? vehiculeJson.VehiculeImages.map((img) => img.urlimage)
        : [],
    };
  });

  res.json({ vehicules: result });
});
